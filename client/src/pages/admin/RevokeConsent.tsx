...
const RevokeConsent:React.FC = () => {
    ...
    const onsubmit = async (data:any) => {
        try {
            const response = await fetch('http://localhost:8085/consent/revoke-consent?patientId=' + data.patientId + '&doctorId=' + data.doctorId, 
                {
                    method:'DELETE'
                }
            );
        ...
        <IonPage>
            ...
            <IonContent className='ion-padding'>
                <form onSubmit={handleSubmit(onsubmit)} className="max-w-6xl mx-auto mt-5">
                    <h1 className='text-center text-2xl font-semibold mb-8'>Revoke consent for patient data</h1>
                    <TextInput name='patientId'
                        placeHolder="Enter patient details to revoke consent"
                        label="Patient ID"
                        control={control}
                    />
                    <TextInput name='doctorId'
